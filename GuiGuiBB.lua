				...
				if getsynasset then
					...
					elseif #image > 14 and string.sub(image, 1, 14) == "synasseturl://" then
						local x, e = pcall(function()
							...
							local fold = isfolder("./BlackLua Scripts")
							if not fold then
								makefolder("./BlackLua Scripts")
							end
							fold = isfolder("./BlackLua Scripts/Themes")
							if not fold then
								makefolder("./BlackLua Scripts/Themes")
							end
							fold = isfolder(".BlackLua Scripts/Themes/SynapseAssetsCache")
							if not fold then
								makefolder("./BlackLua Scripts Themes/SynapseAssetsCache")
							end
							if not fold or not isfile("./BlackLua Scripts/Themes/SynapseAssetsCache/" .. codename .. ".dat") then
								local res = game:HttpGet(string.sub(image, 15))
								if res ~= nil then
									writefile("./BlackLua Scripts/Themes/SynapseAssetsCache/" .. codename .. ".dat", res)
								end
							end
							return getsynasset(readfile("./BlackLua Scripts/Themes/SynapseAssetsCache/" .. codename .. ".dat"))
						end)
						if x and e ~= nil then
							return e
						end
...
return library, library_flags, library.subs Themes",
			Flags = flags,
			Suffix = "Theme",
			Desginer = true
		}}, {"AddTextbox", "__Designer.Textbox.WorkspaceName", filessection, {
